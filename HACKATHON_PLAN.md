...
1. Resurrect Vosk STT + Piper TTS from git history (commit `4cd88d7`-era files, install scripts already exist) behind a **network-state detector**: online → Gradium voice; offline → local voice. Gemma path never changes.
2. UI badge: **"● All inference on-device"** plus mode indicator ("Voice: Gradium (online)" / "Voice: Local (offline)"). Judges must *see* the claim.
3. Rehearse the Wi-Fi-off transition until it's boringly reliable. Cache a weather/tool failure gracefully offline (tools that need网 should say so, not hang).



### P0 — Compliance & repo (est. 1.5 h — do not skip, this is a disqualification lever)

1. **"New Work Only" transparency:** the rules disqualify anything where judges can't identify event-built work. The repo has a pre-event-looking "Base" commit. Add a prominent README section — **"Built during RAISE 2026 (July 4–5)"** — listing exactly what was created at the event (Gradium streaming voice integration, cross-platform/mac support, Gemma 4 on-device migration, offline mode, Alexa/Android remote bridge work, SSL fix), with commit links. Say it in the video too ("this weekend we…"). Honesty here is cheap; ambiguity is fatal.
2. README rewrite for judges: hero GIF, 3-step run (`ollama pull gemma4:12b` → `npm install` → `npm run dev`), architecture diagram (mic → openWakeWord → Gemma 4 (Ollama, local) → tools → TTS), privacy table (what runs where), Gemma-4-specific features used (function calling, vision, 256K ctx, system role). Repo must be **public**.
3. `.env.example` + API_KEYS_SETUP.txt already handle keys — confirm no secrets in git (`git log -p | grep -i key` spot-check).



### P1 — Demo-strengtheners (est. 2 h)

1. On-screen perf HUD during demo: tokens/sec + TTFT from Ollama's response metrics (`eval_count/eval_duration`) — judges love visible numbers; `showPerformanceStats` flag already exists.
2. Android remote / Echo node demo beat: "any old phone becomes a private smart speaker" (the bridge already exists; show one round-trip).
3. Persistent local memory beat: "remember that my girlfriend's birthday is March 3rd" → restart app offline → recall. Memory + privacy in one beat.



### P2 — Only if ahead of schedule

1. Native audio-in to Gemma E4B (skip STT entirely) — wow factor, unproven via Ollama; timebox 1 h of investigation max.
2. MLX variant (`gemma4:12b-mlx`) A/B for tok/s bragging rights.
3. Landing page (Gemma Vision had one; nice-to-have, not judged directly).
...
| Layer                  | Behavior                                                                                                                                                                                                              |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Instant (zero LLM)** | Weather, time, math, alarms→Calendar, Calendar add/list, memory, open app/site, web search, clipboard, screen vision, folder listing, Spotify play, capabilities — routed by `intentRouter.ts` before Gemma is called |
...
### Seventh pass: Calendar pivot, parallel tools, free web search ✅

Live testing exposed gaps in alarm reliability, compound requests, and web search. Fixes:


| Problem                                                    | Root cause                                                              | Fix                                                                                                                                                                    |
| ---------------------------------------------------------- | ----------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Alarms never appeared in Clock                             | macOS Clock has no scriptable API; opening Clock ≠ creating an alarm    | **Full pivot:** alarm requests create **Calendar events** via AppleScript (silent, no UI activation). Responses say honestly that native Clock alarms aren't supported |
| "Add X's birthday on Wednesday" failed                     | No calendar tool; memory tool captured dated items instead              | New `calendar` local tool + `create_calendar_event` LLM declaration; deterministic routing in `intentRouter.ts` for add/list/schedule queries                          |
| "July 8th" follow-up lost context                          | Date-only replies didn't link to prior title                            | `recentUserText` in routing context + `extractRecentCalendarTitle` for contextual recovery                                                                             |
| "Weather and add Mbappe's birthday…" only returned weather | Compound prompts collected one direct tool; STT heard "added" not "add" | `collectInstantInvocations` gathers all matches; `Promise.all` parallel execution in `runDirectLocalTool`; regex expanded for `added` STT mishearing                   |
| "Research best budget laptops" → model claimed no search   | LLM ignored `deep_research` / `web_search`                              | `shouldAutoDeepResearch()` forces research/compare prompts into the deep-research loop; system prompt + tool descriptions updated                                      |
| Brave Search MCP required paid API key                     | Paid dependency for core feature                                        | Removed brave-search MCP; **free DuckDuckGo HTML parsing** is the default in `localTools.ts` and Pi bridge                                                             |
| `example.com/products?sort=budget` opened wrong folder     | URL patterns matched folder intent first                                | `looksLikeExplicitWebsiteTarget` prioritizes `open_website` before folder routing                                                                                      |
| Garbled alarm STT ("set in alarm", "sudden alarm")         | Normalization gaps                                                      | Expanded `ALARM_PHRASE_FIXES` in `voiceTranscript.ts`; compact time parsing (`448`, spoken numbers)                                                                    |


Calendar listing ("What do I have in the morning on Wednesday?") and weekday/date parsing (`parseCalendarDate`, `WEEKDAY_INDEX`) ship with macOS AppleScript integration in `systemAlarm.ts`. Tests: **189 TS pass** (1 unrelated `config.test.ts` delta assertion still flaky).
...
| 7   | Tool verification (weather, calendar, alarm→Calendar, Spotify, etc.) | ✅ 189 TS tests passing                       |
...
Brain stays **100% local Gemma**. Web search uses free DuckDuckGo HTML parsing (no API key). MCP file tools are optional online enhancements layered on top — same honest hybrid story as Gemma Vision winners.
...
| t       | Beat                                                                                                                                                                                                                                                           |
| ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 18–30 s | **Tool call:** Calendar add ("add X's birthday on December 20th") + screen vision + **"research the best…"** → `deep_research` loops locally then answers. Compound beat: "what's the temperature and add…" fires weather + calendar in parallel               |
...
| 18–30 s | Tool call: "Add Bob's birthday on October 30th" + "What's on my screen?" → Calendar event + Gemma 4 vision answers locally. Optional compound: weather + calendar in one breath. |
...
- [https://ollama.com/library/gemma4](https://ollama.com/library/gemma4) — sizes, tools/vision/audio tags, benchmarks, sampling params
- [https://ai.google.dev/gemma/docs/core](https://ai.google.dev/gemma/docs/core) — Gemma 4 overview, memory table, QAT routing
- [https://developers.google.com/edge/litert-lm/models/gemma-4](https://developers.google.com/edge/litert-lm/models/gemma-4) — M4 tok/s benchmarks, MTP
- [https://blog.google/innovation-and-ai/technology/developers-tools/quantization-aware-training-gemma-4/](https://blog.google/innovation-and-ai/technology/developers-tools/quantization-aware-training-gemma-4/) — QAT sizes (E2B <1 GB)
- [https://ai.google.dev/gemma/docs/capabilities/text/function-calling-gemma4](https://ai.google.dev/gemma/docs/capabilities/text/function-calling-gemma4) — function-calling format + parser
- [https://blog.google/innovation-and-ai/technology/developers-tools/developers-changing-lives-with-gemma-3n/](https://blog.google/innovation-and-ai/technology/developers-tools/developers-changing-lives-with-gemma-3n/) — winner profiles incl. Gemini→Gemma 2nd place
- [https://www.kaggle.com/competitions/google-gemma-3n-hackathon/writeups/gemma-vision](https://www.kaggle.com/competitions/google-gemma-3n-hackathon/writeups/gemma-vision) — 1st-place writeup (hybrid engineering, latency story, real user)
- [https://blog.jetbrains.com/ai/2026/06/how-to-win-a-hackathon-notes-from-the-judging-table/](https://blog.jetbrains.com/ai/2026/06/how-to-win-a-hackathon-notes-from-the-judging-table/) — judge consensus (problem-first, one flow, 90 s)
- [https://info.devpost.com/blog/6-tips-for-making-a-hackathon-demo-video](https://info.devpost.com/blog/6-tips-for-making-a-hackathon-demo-video) — video mechanics
- [https://unsloth.ai/docs/models/gemma-4](https://unsloth.ai/docs/models/gemma-4) · [https://huggingface.co/collections/google/gemma-4-qat-q4-0](https://huggingface.co/collections/google/gemma-4-qat-q4-0) · [https://github.com/moonshine-ai/moonshine](https://github.com/moonshine-ai/moonshine) — supporting references
